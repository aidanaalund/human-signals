...
SINGLE_FMT = "<BBHIhhhhhh"
SINGLE_SIZE = struct.calcsize(SINGLE_FMT)   # 20 bytes
BATCH_SAMPLES = 2
BATCH_SIZE = SINGLE_SIZE * BATCH_SAMPLES    # 80 bytes
...
class BLEIMUReceiver:
    ...
    def _append_one_sample(
        ...
        with self.buffers.lock:
            if self.buffers.t0_us is None:
                self.buffers.t0_us = t_us

            t = (t_us - self.buffers.t0_us) * 1e-6
            ...
            if self.buffers.report_t0 is None:
                self.buffers.report_t0 = now_host
                self.buffers.report_count0 = count
            else:
                dt = now_host - self.buffers.report_t0
    ...
    def _handle_packet(self, data: bytearray):
        ...
        for i in range(BATCH_SAMPLES):
            chunk = data[i * SINGLE_SIZE:(i + 1) * SINGLE_SIZE]
...
class IMUWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("IMU BLE Plotter (416 Hz batched)")
