    ...
    def _handle_packet(self, data: bytearray):
        ...
        with self.buffers.lock:
            ...
            if self.buffers.recording_active:
                self.buffers.capture_samples.append({
                    "count": count,
                    "t_us_mcu": t_us,
                    "t_host_s": t_host,
                    "ax": ax,
...
class IMUWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("IMU Prompted Dataset Collector (BLE)")
        ...
        self.prompt_index = 3563 # 2956 -> 2955 (Write V)
    ...
    def update_plots(self):
        ...
        self.status.setText(
            f"{conn_text} | {status_text} | {rec_text} | "
            f"Prompt {min(self.prompt_index + 1, len(self.prompts))}/{len(self.prompts)} | "
